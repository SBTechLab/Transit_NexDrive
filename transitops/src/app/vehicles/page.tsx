...
export default async function VehiclesPage({
  ...
  searchParams: Promise<{ type?: string; status?: string; region?: string; search?: string; sort?: string; page?: string }>
}) {
  ...
  const where = {
    ...
    ...(params.search && {
      OR: [
        { registrationNumber: { contains: params.search, mode: 'insensitive' as const } },
  ...
  const [vehicles, total, regions] = await Promise.all([
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">{total} vehicles found</p>
        </div>
        <AddVehicleButton />
      </div>
