...
export function VehicleFilters({ regions, currentType, currentStatus, currentRegion }: {
  regions: string[]
  currentType?: string
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      {(currentType || currentStatus || currentRegion) && (
        <button onClick={() => router.push('/vehicles')} className="h-9 px-3 rounded-xl text-sm text-red-500 border border-red-200 bg-red-50 hover:bg-red-100 transition-colors">Clear</button>
      )}
    </div>
  )
}
